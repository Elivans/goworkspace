...
func RepalceRegexp(expr string) (result string) {
	funName := "regexp"
	nums := 2
	var params []string
	for i := 0; i < nums; i++ {
	...
	idx := strings.Index(expr, funName)
	if idx == -1 {
		result = expr
		return
	}
	param_idx := 0
	param_start := false
	for i := idx + len(funName); i < len(expr); i++ {
		str2repalce = str2repalce + string(expr[i])
		if (expr[i] == ' ' || expr[i] == '(') && !param_start {
			continue
		}
		if expr[i] == '"' && !param_start {
			param_start = true
			continue
		}
		if expr[i] == '"' && param_start {
		...
		if param_start {
			params[param_idx] = params[param_idx] + string(expr[i])
		}

	}

	newexpr := expr
	match, _ := regexp.MatchString(params[0], params[1])
	if match {
		newexpr = strings.Replace(newexpr, str2repalce, "1==1", 1)
	} else {
		newexpr = strings.Replace(newexpr, str2repalce, "1==2", 1)
	}
	fmt.Println(newexpr)
	result = RepalceRegexp(newexpr)
	return
}

func MustCompile2(expr string) (*Expression, error) {
	expr2 := RepalceRegexp(expr)
	m, err := Compile(expr2)

	return m, err
}

func MustCompile(expr string) (*Expression, error) {
	m, err := Compile(expr)
