...
#cgo LDFLAGS: -LC:/golang/mingw64/lib -lcyoracle
#include <boss.h>
*/
import "C"
import (
...
func (dbi *DB) TableColumns(tabname string) *Columns {
	...
	var err error
	cols.Cols, err = dbi.SelectList("SELECT lower(column_name) FROM user_tab_columns WHERE table_name=UPPER(:1) ORDER BY column_id", cols.Tabname)
	if err != nil || cols.Cols == nil {
		return nil
	}

	cols.Prikey, err = dbi.Get("SELECT lower(column_name) FROM user_ind_columns WHERE table_name=UPPER(:1) AND index_name IN (SELECT constraint_name FROM user_constraints WHERE table_name=UPPER(:2) AND constraint_type='P') AND ROWNUM=1", cols.Tabname, cols.Tabname)
	if err != nil {
...
func (dbi *DB) TableExists(tabname string) bool {
	tabNums, _ := dbi.Get("SELECT 1 FROM user_tables WHERE table_name=UPPER(:1)", tabname)
	if tabNums == "1" {
		return true
	}
	return false
